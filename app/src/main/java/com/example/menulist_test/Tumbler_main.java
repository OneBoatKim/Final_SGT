...
public class Tumbler_main extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tname.setTypeface(Typeface.createFromAsset(getAssets(),"RIXGOL.TTF"));
        ...
        linearLayout = (LinearLayout)findViewById(R.id.tumbler_main_blur);
    ...
    public void go_to_addmoney(View v){
        if(tid.equals("")){
            Toast.makeText(getApplicationContext(), "등록된 텀블러가 없습니다.", Toast.LENGTH_LONG).show();
        }else{
            if(tlost.equals("true")){
                Toast.makeText(getApplicationContext(), "분실된 텀블러입니다.", Toast.LENGTH_LONG).show();
            }else{
    ...
    public void go_to_new (View v){
        if(tid.equals("")){
            ...
            Toast.makeText(getApplicationContext(), "이미 등록된 텀블러가 있습니다.", Toast.LENGTH_LONG).show();
        }
            ...
            try{
                ...
                for (int i = 0; i < ja.length(); i++) {
                    ...
                    Picasso.with(getBaseContext())
                            ...
                            .into(imageView);
                    if(jsonObject.getString("lost_yn").equals("false")){
                        linearLayout.setVisibility(View.INVISIBLE);
                    }else if(jsonObject.getString("lost_yn").equals("true")){
                        linearLayout.setVisibility(View.VISIBLE);
                    }else{
                        linearLayout.setVisibility(View.INVISIBLE);
                    }

                    tlost = jsonObject.getString("lost_yn");
                }
            }catch (Exception e){
                e.printStackTrace();
            }finally {
